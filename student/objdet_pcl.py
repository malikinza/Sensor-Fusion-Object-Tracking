...
def show_pcl(pcl):
    ...
    pcd = o3d.geometry.PointCloud() #creating a point cloud instance

    # step 3 : set points in pcd instance by converting the point-cloud into 3d vectors (using open3d function Vector3dVector)
    pcd.points = o3d.utility.Vector3dVector(pcl[:,:3]) #creating point cloud points using the point cloud data first three channels: x,y,z
    ...
    vis.register_key_callback(32, exit_callback)
    vis.update_renderer()
    vis.poll_events()
    vis.run()
...
def show_range_image(frame, lidar_name):
    ...
    # Crop stacked image to +/- 90 deg. left and right of the forward-facing x-axis

    stacked_image = np.vstack((ri_range, ri_intensity))
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[:,1] = np.abs(np.int_(np.floor(lidar_pcl_cpy[:,1] / bev_discrete)) + (configs.bev_width + 1)/2)
    ...
    #capping intensity values by equating outliers to 95th percentile
    upper_bound = np.quantile(lidar_pcl_cpy[:, 3], 0.95)
    lidar_pcl_cpy[lidar_pcl_cpy[:, 3] > upper_bound, 3] = upper_bound

    # step 2 : re-arrange elements in lidar_pcl_cpy by sorting first by x, then y, then -z (use numpy.lexsort)
    idx_height = np.lexsort((-lidar_pcl_cpy[:, 2], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0])) #primary key is x, then y, then z
    ...
    lidar_pcl_top = lidar_pcl_cpy[indices] #keeping only the x,y,z for the top-most z-coordinate

    ## step 4 : assign the intensity value of each unique entry in lidar_top_pcl to the intensity map 
    ##          make sure that the intensity is scaled in such a way that objects of interest (e.g. vehicles) are clearly visible    
    ##          also, make sure that the influence of outliers is mitigated by normalizing intensity on the difference between the max. and min. value within the point cloud
    # assigns an intensity value to an (x,y) in the intensity map, currently filled with zeros
    ...
    cv2.imshow('height_intensity', height_intensity)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
