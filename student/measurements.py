...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        ############
        # TODO Step 4: implement a function that returns True if x lies in the sensor's field of view, otherwise False.
        ############
        #converting x in vehicle coordianates to sensor coordinates
        pos_veh = np.ones((4, 1))  # homogenous coordinates
        pos_veh[0:3] = x[0:3]  # [x1, x2, x3,1]
        pos_sens = self.veh_to_sens * pos_veh
        pos_sens = pos_sens[0:3] #removing the 1 (4th element)

        if pos_sens[0] > 0:
            angle = np.arctan(pos_sens[1]/pos_sens[0]) #y/x
            if angle > self.fov[0] and angle < self.fov[1]:
                return True #vehicle visible in sensor field of view
            else:
                return False #vehicle not visible
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            #projecting camera coordinates to 2D image plane using pinhole camera model
            hx = np.zeros((2, 1))
            if x == 0:
