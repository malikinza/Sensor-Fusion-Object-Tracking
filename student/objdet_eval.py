...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            label_corners = tools.compute_box_corners(label.box.center_x,
                                             label.box.center_y,
                                             label.box.width,
                                             label.box.length,
                                             label.box.heading)
            # computes location of each corner of a box and returns [front_left, rear_left, rear_right, front_right]
            ...
            for obj in detections:
                ...
                #label_fl, label_rl, label_rr, label_fr = label_corners
                #det_fl, det_rl, det_rr, det_fr = det_corners
                dist_x = label.box.center_x - x
                cen_y = label.box.center_y - y
                cen_z = label.box.center_z - z
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                x1_l, y1_l = label_corners[0][0], label_corners[0][1]
                x2_l, y2_l = label_corners[2][0], label_corners[2][1]
                x1_d, y1_d = det_corners[0][0], det_corners[0][1]
                x2_d, y2_d = det_corners[2][0], det_corners[2][1]
                ...
                if x2I > x1I and y2I > y1I:
                    ...
                    union_area = (y2_l -y1_l)*(x2_l-x1_l) + (y2_d -y1_d)*(x2_d - x1_d)
                    iou = intersection_area / union_area
                    return iou
                else:
                    return 0
                ## step 6 : if IOU exceeds min_iou threshold, store [iou,dist_x, dist_y, dist_z] in matches_lab_det and increase the TP coun
    ...
    ## step 1 : compute the total number of positives present in the scene
     #all positive = ground truth positives
    all_positives = labels_valid.sum()

    ## step 2 : compute the number of false negatives
     #false negative = object is missed
    true_positives = len(ious)  #incase of multiple candidates for a single object
    false_negatives = all_positives - true_positives #ground truth of all positives - detected true positives = missed positives
